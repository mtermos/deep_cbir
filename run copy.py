...
class new_ViT(nn.Module):
    def __init__(self, model):
        ...
        h, w = as_tuple(model.image_size)  # image sizes
        fh, fw = as_tuple(16)  # patch sizes
...
def my_model(model_name):
    ...
    if(model_name == "vit"):
        cnn_model = ViT('B_16_imagenet1k', pretrained=True)
        new_model = new_ViT(cnn_model)
...
def extract_vectors(image_path, bbxs=None):
    ...
    with torch.no_grad():
        ...
        vecs = model(input).cpu().data.squeeze()
           
    return vecs

def extract_vectors_original(image_path, bbxs=None):
    model.cuda()
    model.eval()
